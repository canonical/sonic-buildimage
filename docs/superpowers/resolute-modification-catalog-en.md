...
**Caveats** (1) `platform/vpp` in-submodule variant-naming changes need a separate submodule commit + pointer bump (vs doesn't use vpp, so no impact). (2) `docker-sonic-vs` (bookworm family, findstring guard, not built by default) not migrated. (3) The shared template `docker-*-trixie.mk` was edited directly to reference resolute, meaning under `BLDENV=trixie` building a non-vs platform syncd would point at the resolute base while the trixie base is still pristine — a latent inconsistency; vs build (`BLDENV=resolute`) is unaffected. (4) **Post-migration fix — iproute2 missing in `docker-base-resolute`.** The `docker-base-resolute/Dockerfile.j2` was copied from trixie, which omits `iproute2` from the apt install list (trixie installs it as a locally-built patched deb via `rules/iproute2.mk`). However, `rules/iproute2.mk` guards the `IPROUTE2` variable with `ifeq ($(BLDENV),trixie)` — under `BLDENV=resolute` the variable is empty, and `rules/docker-base-resolute.mk`'s `$(IPROUTE2)` dependency silently expands to nothing. Result: the `ip` command was entirely absent from all resolute containers. Fix: add `iproute2` to the apt install list in `docker-base-resolute/Dockerfile.j2` (matching `docker-base-bookworm`); remove the empty `$(IPROUTE2)` dependency from `rules/docker-base-resolute.mk`. This drops the EVPN Multihoming protocol field patch (not needed for current resolute scope). The same trixie-only `BLDENV` guard pattern also affects `sonic-redfish` (`BMCWEB`/`SONIC_DBUS_BRIDGE` empty in resolute, bmcweb/sonic-dbus-bridge silently missing) and `platform/vpp` (`VPPINFRA` not built, only affects VPP platform builds); these are known but not yet fixed.
