...
### baked image 完整 T0 汇总(2026-07-17)—— 87 pass
xml 确认(自然完成,junit-xml):43P/9F/7E/21S
log 进度统计(timeout 但 -q 字符捕获):dhcp_relay 23P/10F/1E/4S,arp 13P/10E/1S,bgp 8P/16F/20E/2S
总计:**87 pass / 35 fail / 38 err / 28 skip**(160 executed)

| 目录 | pass | fail | err | skip | 来源 | 备注 |
|---|---|---|---|---|---|---|
| bgp_fact | 1 | 0 | 0 | 0 | xml | 干净 |
| lldp | 10 | 0 | 0 | 1 | xml | 干净 |
| vlan | 8 | 0 | 1 | 4 | xml | 1 err(非 resolute) |
| pc(lag) | 6 | 7 | 0 | 12 | xml | fail = po_voq 多 ASIC / lag_member(拓扑) |
| snmp | 18 | 1 | 6 | 3 | xml | 6 err = VS 无传感器 KeyError |
| container_checker | 0 | 1 | 0 | 1 | xml | telemetry 变体 fail |
| dhcp_relay | 23 | 10 | 1 | 4 | log | timeout 4800;23 pass |
| arp | 13 | 0 | 10 | 1 | log | timeout 1800;10 err = neighbor_mac_noptf KeyError/PTF |
| bgp | 8 | 16 | 20 | 2 | log | timeout 10800;20 err = reliable_tsa setup 级联 |
| **合计** | **87** | **35** | **38** | **28** | | |

38 err 全非 resolute:bgp reliable_tsa setup 级联(多 DUT)、arp neighbor_mac_noptf KeyError/PTF、snmp VS 无传感器 KeyError、1 dhcp err。DUT 全程健康(BGP 4 Estab)。
bgp/dhcp_relay/arp timeout = 测试量大 + 慢/hang 测试(整批 fixture 累积,非 resolute)。acl = 真 hang(goal 排除)。
修复前(原始):70 pass / 47 fail / 1043 err / 651 skip —— err 主要是 5 个 resolute bug 导致的 broken-pipe 级联。修复后:87 pass,38 err(全非 resolute),级联消除。

关键:5 个 resolute build 修复消除了 1043-error 级联。剩余 err/fail 是测试床/测试框架特定问题(多 DUT、VS 无传感器、PTF 数据面),非 resolute 回归。大目录 timeout 是测试量大,非 resolute。
