...
### baked image 干净 T0 结果(2026-07-16,长 timeout)
5 修复 baked DUT 上,跑完的目录(junit-xml,跳破坏性,长 timeout):

| 目录 | pass | fail | err | skip | 备注 |
|---|---|---|---|---|---|
| bgp_fact | 1 | 0 | 0 | 0 | 干净 |
| lldp | 10 | 0 | 0 | 1 | 干净(1 reboot 变体 skip) |
| vlan | 8 | 0 | 1 | 4 | 8 pass;1 err(待查);4 破坏性 skip |
| pc(lag) | 6 | 7 | 0 | 12 | 7 fail = po_voq(多 ASIC)/lag_member_forwarding —— 拓扑,非 resolute |
| snmp | 18 | 1 | 6 | 3 | 6 err = KeyError phy_entity(VS 无物理传感器);vs r1-snmp 14P/10E → baked 18P/6E(更好) |
| container_checker | 0 | 1 | 0 | 1 | 1 fail = telemetry 变体(非 resolute) |
| **合计** | **43** | **9** | **7** | **21** | 级联基本消除(7 err,对比修复前几百) |

跑 timeout 的目录(大/慢,非 resolute —— DUT 全程健康):
- **bgp 全量**(60 文件):timeout 5400s 仍未完——太大,某测试慢/hang。原始 11P/23F/196E(多 broken-pipe 级联,现已消除)。
- **arp**:timeout 1200/1800——PTF 状态累积,慢;部分 -v 数据:arp_update 5P,arpall 2P+3F(ARP reply 数据面,非 resolute)。
- **dhcp_relay**:timeout(某测试慢)。
- **acl**:真 hang/deadlock(SystemExit/-2),按 goal 排除。

关键:1043-error 级联根因是 5 个 resolute bug 使 DUT 不稳定;修复后,跑完的目录只 7 err(全非 resolute:VS 无传感器 KeyError、ARP 数据面)。大目录 timeout 是测试量大/PTF 慢,非 resolute 回归。
