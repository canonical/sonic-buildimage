...
## 2. resolute 构建 bug(在 DUT 上临时修复,未 commit/push)

正式修复应在 build 侧。

| # | bug | 症状 | 根因 | 修复 |
|---|---|---|---|---|
| 1 | teamd 容器缺 iproute2 | PortChannel DOWN,BGP Idle(NoIf) | teammgrd 跑 `ip link set ... master` 但 docker-teamd 镜像无 `ip` 命令 | 装 iproute2+libbpf1+libmnl0+libelf1t64(scp deb→docker cp→dpkg -i),重启 teamd |
| 2a | show 插件 hyphen import | "failed to import plugin" 警告污染 stdout | 7 个插件名含 hyphen(dhcp-relay/cisco-8000/sonic-*),`importlib.import_module` 不接受 hyphen | 改 `util_base.py` load_plugins:hyphen 名用 `spec_from_file_location` 按路径加载 |
| 2b | show 插件 install-path 损坏 | dhcp-relay.py / macsec.py 是套娃空目录 | 构建 install bug:绝对路径当相对路径,文件装成 `目标/绝对路径` 套娃目录 | 把套娃底部的真文件移到正确路径 |
| 3 | snmpd 绑 IPv6 失败 | snmpd exit 1,FATAL | mgmt IPv6 `fec0::ffff:afa:1` 没配到 eth0(deploy-mg 没配) | 手动 `ip -6 addr add fec0::ffff:afa:1/64 dev eth0` |
| 4 | sonic_ax_impl py3.14 不兼容 | snmp-subagent exit 1 | `asyncio.get_event_loop()` 在 py3.14 抛 RuntimeError(无 current event loop) | 改 main.py:20 `get_event_loop`→`new_event_loop` |
| 5(额外) | deploy-mg sshd restart 失败 | deploy-mg 末尾 exit 2 | resolute 用 `ssh.service`,playbook 调 `systemctl restart sshd` | playbook 1382 行 `sshd`→`ssh` |

**可复用修复脚本:** `/tmp/apply-resolute-fixes.sh`(重应用 1-4;scp 到 DUT + `sudo bash`)。每次测试床重建后重跑(add-topo 重建 DUT 会丢运行时修复)。
