...
### baked image 完整 T0 grand total(2026-07-18)—— 3870 pass
在 5 修复 baked DUT 上跑了**所有 T0 目录**(每个含 test_*.py 的目录),跳破坏性(-k),长 timeout,proxy 192.168.1.105:6152。从 junit-xml(自然完成)+ log 进度(-q 字符,timeout 批次)汇总:
...
批次亮点:small6(log_fidelity+logs+macsec+monit+...)1268P;small7(nat+ntp+ptftests+...)969P;small11(pfcwd+gnmi_e2e+k8s+...)918P;small9(acstests+bmp+ecmp+fib+...)377P;small1(sub_intf+copp+everflow+fdb)105P;small2(mgmt+qos+route+sensors)105P;snmp 18P;lldp 10P;vlan 8P;dhcp 23P;arp 13P;bgp 8P;platform_tests 5P;acl 0P(hang)。

760 err 全非 resolute:platform_tests 316(VS 无物理 SFP/PSU/sensor API)、ptftests 260(PTF setup)、small9 57、bgp 20(reliable_tsa 多 DUT 级联)、arp 10(neighbor_mac_noptf KeyError)。DUT 全程健康(BGP 4 Estab)。
335 fail:small6 224(macsec/mclag/mpls 数据面)、small7 34、small9 3、pc 7(po_voq 多 ASIC)、dhcp 10 —— 拓扑/数据面特定。
acl = 真 hang(SystemExit/-2,3E,排除)。reboot = 破坏性(排除)。dualtor = dual-tor 拓扑(多 skip)。

对比:修复前(原始)70 pass / 47 fail / **1043 err** / 651 skip → 修复后 **3870 pass** / 335 fail / **760 err** / 8525 skip。
Pass 70→3870(55 倍)。1043-error broken-pipe 级联(5 个 resolute bug 导致)已消除;剩余 760 err 是 VS 平台/测试框架特定问题,非 resolute 回归。

关键:5 个 resolute build 修复(teamd iproute2、ssg snprintf、sonic_ax_impl asyncio、sonic-utilities hyphen+dockerapi、deploy-mg sshd→ssh)消除了曾阻塞几乎所有测试的级联。DUT 稳定后,所有 T0 目录共 3870 测试通过。
