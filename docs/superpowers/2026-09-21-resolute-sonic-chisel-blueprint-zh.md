...
| 要多久 | **8 至 16 个月**，由上游评审吞吐决定（26.04 每周合入 1.9 个 PR、已积压 39 个）。编写本身可由 AI 完成，不是约束项 |
...
3. **减少要上游的数量。** 不是所有 66 个都必须进上游。低价值的包（只服务一个容器、体积收益很小的）可以长期留在我们的 fork 里。**把「必须上游」和「fork 够用」分开，是唯一能直接缩短关键路径的技术手段。** 这个分类本文尚未做，应当在开工前做完。
