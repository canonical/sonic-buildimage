...
| 最大的风险？ | 上游评审吞吐，这是唯一的关键路径。能真正加速的只有三件事，见 5.3 |
| 要多久 | 只推真正需要上游的 21 个包（5.4）是 **2.6 个月**，全推 66 个是 8.1 个月。由上游评审吞吐决定（26.04 每周 1.9 个 PR、积压 39 个）。编写可由 AI 完成，不是约束项 |
...
| 其中 26.04 | **39 个 open**，89% 卡在 `REVIEW_REQUIRED`（等评审，不是等作者） |
| PR 形态 | 改动文件中位 **2 个**（正好是 SDF + spread 测试），100 个里只有 2 个标题含并列 |
| 贡献者集中度 | top1 一人占 48/100 |

**PR 基本不打包多个包。** 文件数大的 PR（61 文件的 `gcc`、23 文件的 `binutils`）是单个包跨多架构，不是多个包合并提交。所以「一个包一个 PR」不是我们的选择，是上游的既成形态。

### 5.2 由此得出的日历时间

编写不是约束。**SDF 的编写、测试、自检整套流程已经可以由 AI 跑完**（chisel-slicer skill 的 10 步工作流即为此设计，止于 commit，PR 由人开）。所以产能取决于上游能吸收多快：

| 情形 | 周数 | 月数 |
|---|--:|--:|
| 清空 26.04 现有积压（39 个） | 21 | 4.8 |
| 我们 66 个 PR，独占该分支全部产能 | 35 | 8.1 |
| 我们只拿到一半产能 | 69 | 16.2 |

**所以这件事的量级是「按月算」，不是「7 人周」。** 之前写的 35 人日只描述了编写，而编写恰恰是最不稀缺的部分。

### 5.3 唯一真正能加速的三件事

按杠杆大小排：

1. **建立贡献者信用。** 实测差距极大：top3 作者的合入延迟中位 **3.0 天**，而只提交过 1 至 2 个 PR 的作者中位 **18 天**，差 6 倍。jy5275 已有 4 个合入 PR，是现成的起点。**前几个 PR 必须又小又干净**，这不只是「趟流程」，它直接决定后面 60 个的速度。
2. **争取评审承诺。** 39 个积压、89% 等评审、top1 一人占近半合入量，说明评审产能是稀缺资源而非自动服务。与其把 66 个 PR 扔进公共队列，不如先谈一个安排：批量评审窗口、指定评审人、或者我们这边也承担一部分评审。**这是一次对话，可能比任何技术优化都有效。**
3. **减少要上游的数量。** 不是所有 66 个都必须进上游。**这是唯一能直接缩短关键路径的技术手段，分类见 5.4，做完之后关键路径从 8 个月压到 1.6 至 2.6 个月。**

不管用的：多雇人写 SDF、把批次拆得更细、并行开更多 PR。这些都在加速已经不稀缺的那一端。


### 5.4 哪些必须上游，哪些 fork 够用

**判据不是体积。** 实测这 65 个待办包合计只有 60 MB，其中文档/man/locale 仅占 6%——切片能省的绝对体积很小，「省空间」撑不起 66 个 PR 的代价。

正确的问题是：**一个 slice 留在 fork 里，我们永久损失什么？** 按 4.5 查明的机制，fork-only 意味着配方必须走 `override-build` + `chisel cut --release` 而不是 `stage-packages`。**这个代价按「有多少个配方要引用它」放大**，所以判据是容器数；其次看上游胃口（归档里有多少包依赖它，`rdep`），因为那决定评审阻力。

#### 第 1 档：必须上游（13 个）

出现在 10 个以上容器里。fork-only 的复杂度会乘以容器数，不划算。
...
注：`net-tools`、`libdaemon0`、`libpopt0` 的容器数是 30，因为它们已在 base 层；若按 3.3 的默认值把前两个判为不需要，本档降到 11 个。

#### 第 2 档：值得上游（8 个）

容器数不多，但归档里 50 个以上的包依赖它——**上游本来就想要这个 slice，评审阻力最小**，等于用别人的意愿换我们的进度。
...
`kmod` 有 1279 个包依赖却还没 SDF，是明显的空白，上游大概率乐意收。

#### 第 3 档：fork 够用（45 个）

单容器或双容器、且 `rdep` 低于 50。典型是 `radvd`（2）、`ndisc6`（0）、`ndppd`（0）、`python3-smbus`（1）、`i2c-tools`（2）——只有 SONiC 会用，上游收了也没人消费。

**fork 的边际成本接近零**：`override-build` 那套机械一旦搭好（批 0 就要做），加第 10 个和第 45 个 fork-only slice 的额外成本几乎一样。所以长尾放 fork 是对的。

#### 三种切法的关键路径

| 方案 | PR 数 | 独占 26.04 产能 |
|---|--:|---|
| 只推第 1 档 | 13 | 7 周 ≈ **1.6 个月** |
| 推第 1 + 2 档 | 21 | 11 周 ≈ **2.6 个月** |
| 全推 | 66 | 35 周 ≈ 8.1 个月 |

**建议推第 1 + 2 档，21 个。** 第 1 档是不得不做的，第 2 档是顺水人情且能积累贡献者信用（5.3 第 1 条），两者合计把关键路径砍掉三分之二。第 3 档随时可以事后补推——**fork 转上游是单向可逆的，反过来不是**，所以先少推没有风险。

这个分类需要一次产品确认：**长期维护 45 个 fork-only slice 的成本**（持续 rebase、配方里的 `override-build` 复杂度、上游哪天自己加了同名 SDF 时的冲突）**是否低于多等 5.5 个月**。本文的判断是低于，但这是可以推翻的。

### 5.5 提交队列，而不是批次

因为瓶颈在评审，正确的调度是**维持一个恒定的在途 PR 数**，而不是分批投放。建议：

- **在途上限 5 至 8 个**。多了只会堆在队列里变老，还会稀释评审者对我们的注意力。
- **顺序由两个约束定**：依赖必须先合入（chisel 解析 `essential:`，叶子优先是 skill 的 Step 2 硬要求），以及优先解锁最多容器的包。
- **合入一个补一个**，保持管道满而不溢。

下面这张表因此是**提交顺序**，不是工作分批：

| 批 | 包 | 项数 | 说明 |
|---|---|--:|---|
| 0 | `util-linux`（补 `logger` slice） | 1 | **最先做**，用最简单的改动趟通 CLA、CI 和评审。**批 0 的 PR 拿到上游第一轮反馈之前不要开批 1**，否则八个批次会带着同一类问题一起被打回 |
| 1 | `rsyslog`、`libestr0`、`libfastjson4`、`rsyslog-relp`、`librelp0` | 5 | rsyslog 全家。前三个从 24.04 移植，后两个新写。`rsyslog-relp` 依赖 `librelp0`，必须同批否则 `essential:` 解析不了 |
| 2 | `libpython3.14`、`python3-yaml`、`python3-redis`、`python3-cffi-backend` | 4 | 结构相似，可批量化 |
| 3 | `radvd`，以及 `net-tools`、`libdaemon0`（若确认需要） | 1 至 3 | 见下方默认值 |
| 4 | `arping`、`bridge-utils`、`conntrack`、`dmidecode`、`ifupdown`、`libibverbs1`、`libnet9`、`libpcap0.8t64`、`libpci3`、`libprotobuf32t64`、`ndisc6`、`ndppd`、`pci.ids`、`pciutils`、`python3-netifaces`、`python3-protobuf`、`tcpdump` | 17 | orchagent / nat / teamd / sflow / macsec / dash-ha 一族，顺带做掉多容器共用包 |
| 5 | `cron-daemon-common`、`libgoogle-perftools4t64`、`liblsof0`、`libpcre2-posix3`、`libpopt0`、`libsnmp-base`、`libsnmp40t64`、`libtcmalloc-minimal4t64`、`logrotate`、`lsof` | 10 | fpm-frr。`libpopt0` 是 `logrotate` 的硬依赖，同批 |
| 6 | `freeipmi-common`、`ipmitool`、`libfreeipmi17`、`snmp`、`snmpd` | 5 | snmp、lldp。snmp 库已在批 5 做掉 |
| 7 | `ethtool`、`i2c-tools`、`libdbi1t64`、`libi2c0`、`libnvme1t64`、`librrd8t64`、`nvme-cli`、`psmisc`、`python3-bottle`、`python3-smbus`、`rrdtool`、`smartmontools`、`udev`、`uuid-runtime`、`xxd` | 15 | platform-monitor，**最重的一批**，`udev`、`rrdtool`、`smartmontools` 都带配置，放在手感练出来之后 |
| 8 | `ibverbs-providers`、`kmod`、`libdbus-c++-1-0v5`、`libexplain51t64`、`libjsoncpp26`、`libprotobuf-lite32t64`、`lz4` | 7 | dhcp-relay、sysmgr、syncd-brcm、gbsyncd-agera2/broncos/credo。不含 `gbsyncd-vs` |

顺序 0 至 3 合计 11 至 13 项（A 桶 1 + B 桶 3 + C 桶 7 至 9），批 4 至 8 合计 54 项，总计 65 至 67。包名按「第一个需要它的批次」归属，所以批 4 偏大。

批 4 往后的数字是上界（3.4），下游配方写完后按 2.2 的已迁移口径重算，实际很可能显著更低。

**`net-tools` 和 `libdaemon0` 给默认值**：Docker 侧证据是全仓库零运行期**文本**调用者。**注意这个判据对共享库是无效的**——`libdaemon0` 是 `.so`，不会有人在脚本里写它的名字，正确判法是对 primed rootfs 做 ELF `DT_NEEDED` 闭包并查 `dlopen`。本轮没做这一步。**默认取不需要，按不写排期**；若下游确认确有用途再补，代价是两个 SDF。不要停在这里等答复。
...
| 项 | 性质 | 应对 |
|---|---|---|
| 上游评审吞吐 | **唯一的关键路径**（5.1 已实测） | 26.04 每周合入 1.9 个 PR、积压 39 个。应对见 5.3：建信用、谈评审安排、缩减上游数量。两级验收（4.5）和 fork 消费只是让我们不空等，并不加速合入 |
...
**真正的排期数见 5.2：8 至 16 个月，取决于我们能拿到多少上游评审产能。** 两者差两个数量级，所以任何关于人力投入的讨论都应当先回答 5.3 的三个问题。
...
| 决定哪些包不必上游（5.3 第 3 条） | 需要产品判断：体积收益 vs 长期维护 fork 的成本 |
