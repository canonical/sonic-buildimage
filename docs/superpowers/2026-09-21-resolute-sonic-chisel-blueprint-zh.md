...
| 工作量 | 约 **35 人日**，不含上游评审往返，且排除项全部朝偏大方向 |
...
2. **排除项全部朝一个方向偏**：不含评审返工、不含跨 6 架构的 CI 失败修复、不含 18 个带配置 SDF 的 spread 测试调试。这三项都只会让数字变大。唯一朝下的因素是批 4 往后的上界重算，而那依赖第二部分的裁剪工作，本文范围之外。**因此 35 人日对人力是合理下限，对日历时间几乎肯定偏低。**
3. 不含第二部分的任何工作。
