/*
 * A hwmon driver for the netweb_i2c_cpld
 *
 * Copyright (C) 2013 Netweb Technology Corporation.
...
static const struct i2c_device_id netweb_i2c_cpld_id[] = {
...
MODULE_DEVICE_TABLE(i2c, netweb_i2c_cpld_id);
...
int netweb_i2c_cpld_read(u8 cpld_addr, u8 reg);
int netweb_i2c_cpld_write(unsigned short cpld_addr, u8 reg, u8 value);
...
/* Addresses scanned for netweb_i2c_cpld
 */
                                ...
                                struct device_attribute *devattr, char *buf)
{
    ...
    while(i < data->sfp_num)
    {
        ...
        if(cpld_addr == client->addr)
           ...
           value = netweb_i2c_cpld_read(cpld_addr, reg);
...
static void netweb_i2c_cpld_add_client(struct i2c_client *client)
{
...
static void netweb_i2c_cpld_remove_client(struct i2c_client *client)
{
...
static int netweb_i2c_cpld_probe(struct i2c_client *client)
{
    int status;
    ...
    dev_id = i2c_match_id(netweb_i2c_cpld_id, client);
    ...
    netweb_i2c_cpld_add_client(client);
...
static void netweb_i2c_cpld_remove(struct i2c_client *client)
{
    struct cpld_data *data = i2c_get_clientdata(client);
    ...
    sysfs_remove_group(&client->dev.kobj, &data->group);
    kfree(data->group.attrs);
    netweb_i2c_cpld_remove_client(client);
}
...
EXPORT_SYMBOL(netweb_i2c_cpld_read);
...
EXPORT_SYMBOL(netweb_i2c_cpld_write);


static struct i2c_driver netweb_i2c_cpld_driver = {
    .class        = I2C_CLASS_HWMON,
    .driver = {
        .name = "netweb_i2c_cpld",
    },
    .probe		= netweb_i2c_cpld_probe,
    .remove	   	= netweb_i2c_cpld_remove,
    .id_table     = netweb_i2c_cpld_id,
    .address_list = normal_i2c,
};


static int __init netweb_i2c_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&netweb_i2c_cpld_driver);
}

static void __exit netweb_i2c_cpld_exit(void)
{
    i2c_del_driver(&netweb_i2c_cpld_driver);
}

module_init(netweb_i2c_cpld_init);
module_exit(netweb_i2c_cpld_exit);

MODULE_AUTHOR("Brandon Chuang <brandon_chuang@netweb.com.tw>");
MODULE_DESCRIPTION("netweb_i2c_cpld driver");
MODULE_LICENSE("GPL");
