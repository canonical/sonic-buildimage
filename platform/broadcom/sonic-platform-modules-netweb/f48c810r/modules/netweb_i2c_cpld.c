/*
 * Copyright (C)  Brandon Chuang <brandon_chuang@netweb.com.tw>
 *
 * This module supports the netweb cpld that hold the channel select
 * mechanism for other i2c slave devices, such as SFP.
 * This includes the:
 *	 Netweb f48c810r CPLD1/CPLD2/CPLD3
 *
 * Based on:
...
struct f48c810r_cpld_data {
    enum cpld_type   type;
    struct device   *hwmon_dev;
    struct mutex     update_lock;
};

static const struct i2c_device_id f48c810r_cpld_id[] = {
    { "f48c810r_cpld1", f48c810r_cpld1 },
    { "f48c810r_cpld2", f48c810r_cpld2 },
    { "f48c810r_cpld3", f48c810r_cpld3 },
    { }
};
MODULE_DEVICE_TABLE(i2c, f48c810r_cpld_id);
...
enum f48c810r_cpld_sysfs_attributes {
	CPLD_VERSION,
	ACCESS,
	MODULE_PRESENT_ALL,
	MODULE_RXLOS_ALL,
...
static ssize_t show_version(struct device *dev, struct device_attribute *da,
             char *buf);
static int f48c810r_cpld_read_internal(struct i2c_client *client, u8 reg);
static int f48c810r_cpld_write_internal(struct i2c_client *client, u8 reg, u8 value);
...
static struct attribute *f48c810r_cpld3_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
	NULL
};

static const struct attribute_group f48c810r_cpld3_group = {
	.attrs = f48c810r_cpld3_attributes,
};

static struct attribute *f48c810r_cpld2_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
	/* transceiver attributes */
...
static const struct attribute_group f48c810r_cpld2_group = {
	.attrs = f48c810r_cpld2_attributes,
};

static struct attribute *f48c810r_cpld1_attributes[] = {
    &sensor_dev_attr_version.dev_attr.attr,
    &sensor_dev_attr_access.dev_attr.attr,
	/* transceiver attributes */
	&sensor_dev_attr_module_present_all.dev_attr.attr,
...
static const struct attribute_group f48c810r_cpld1_group = {
	.attrs = f48c810r_cpld1_attributes,
};
             ...
             char *buf)
{
	...
	struct f48c810r_cpld_data *data = i2c_get_clientdata(client);

    if (data->type == f48c810r_cpld2) {
        regs_p = regs_h1;
    } else {
        regs_p = regs_h2;
    }
    ...
    for (i = 0; i < sizeof(values); i++) {
        status = f48c810r_cpld_read_internal(client, regs_p[i]);
             ...
             char *buf)
{
    ...
    for (i = 0; i < ARRAY_SIZE(regs); i++) {
        status = f48c810r_cpld_read_internal(client, regs[i]);
             ...
             char *buf)
{
    ...
    mutex_lock(&data->update_lock);
	status = f48c810r_cpld_read_internal(client, reg);
			...
			const char *buf, size_t count)
{
	...
	if (disable) {
        ...
        status = f48c810r_cpld_write_internal(client, reg, status);
	if (unlikely(status < 0)) {
		goto exit;
	}
			...
			const char *buf, size_t count)
{
	...
	mutex_lock(&data->update_lock);
	status = f48c810r_cpld_write_internal(client, addr, val);
	if (unlikely(status < 0)) {
		goto exit;
	}
	mutex_unlock(&data->update_lock);
...
static void f48c810r_cpld_add_client(struct i2c_client *client)
{
...
static void f48c810r_cpld_remove_client(struct i2c_client *client)
{
 ...
 */
static int f48c810r_cpld_probe(struct i2c_client *client)
{
	struct i2c_adapter *adap = to_i2c_adapter(client->dev.parent);
	struct f48c810r_cpld_data *data;
	int ret = -ENODEV;
	...
	data = kzalloc(sizeof(struct f48c810r_cpld_data), GFP_KERNEL);
	if (!data) {
		ret = -ENOMEM;
		goto exit;
	}

	id = i2c_match_id(f48c810r_cpld_id, client);
    ...
    switch (data->type) {
    case f48c810r_cpld1:
        group = &f48c810r_cpld1_group;
        break;
    case f48c810r_cpld2:
        group = &f48c810r_cpld2_group;
        break;
	case f48c810r_cpld3:
        group = &f48c810r_cpld3_group;
        break;
    default:
        break;
    }
    ...
    f48c810r_cpld_add_client(client);
    return 0;
...
static void f48c810r_cpld_remove(struct i2c_client *client)
{
    struct f48c810r_cpld_data *data = i2c_get_clientdata(client);
    const struct attribute_group *group = NULL;

    f48c810r_cpld_remove_client(client);
...
int f48c810r_cpld_read(unsigned short cpld_addr, u8 reg)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EPERM;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = f48c810r_cpld_read_internal(cpld_node->client, reg);
    		break;
        }
    }
...
EXPORT_SYMBOL(f48c810r_cpld_read);

int f48c810r_cpld_write(unsigned short cpld_addr, u8 reg, u8 value)
{
    struct list_head   *list_node = NULL;
    struct cpld_client_node *cpld_node = NULL;
    int ret = -EIO;
    ...
    list_for_each(list_node, &cpld_client_list)
    {
        ...
        if (cpld_node->client->addr == cpld_addr) {
            ret = f48c810r_cpld_write_internal(cpld_node->client, reg, value);
            break;
        }
    }
...
EXPORT_SYMBOL(f48c810r_cpld_write);

static struct i2c_driver f48c810r_cpld_driver = {
	.driver		= {
		.name	= "f48c810r_cpld",
		.owner	= THIS_MODULE,
	},
	.probe		= f48c810r_cpld_probe,
	.remove		= f48c810r_cpld_remove,
	.id_table	= f48c810r_cpld_id,
};

static int __init f48c810r_cpld_init(void)
{
    mutex_init(&list_lock);
    return i2c_add_driver(&f48c810r_cpld_driver);
}

static void __exit f48c810r_cpld_exit(void)
{
    i2c_del_driver(&f48c810r_cpld_driver);
}

MODULE_AUTHOR("Brandon Chuang <brandon_chuang@netweb.com.tw>");
MODULE_DESCRIPTION("Netweb I2C CPLD driver");
MODULE_LICENSE("GPL");

module_init(f48c810r_cpld_init);
module_exit(f48c810r_cpld_exit);
