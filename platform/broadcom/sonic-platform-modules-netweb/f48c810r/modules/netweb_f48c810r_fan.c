/*
 * A hwmon driver for the Netweb as7326 56x fan
 *
...
static struct f48c810r_fan_data *f48c810r_fan_update_device(struct device *dev);
...
extern int netweb_i2c_cpld_read(unsigned short cpld_addr, u8 reg);
extern int netweb_i2c_cpld_write(unsigned short cpld_addr, u8 reg, u8 value);
...
static struct attribute *f48c810r_fan_attributes[] = {
...
static int f48c810r_fan_read_value(struct i2c_client *client, u8 reg)
{
    return i2c_smbus_read_byte_data(client, reg);
}

static int f48c810r_fan_write_value(struct i2c_client *client, u8 reg, u8 value)
{
...
static u8 is_fan_fault(struct f48c810r_fan_data *data, enum fan_id id)
{
    u8 ret = 1;
...
static ssize_t set_enable(struct device *dev, struct device_attribute *da,
                          const char *buf, size_t count)
{
    struct f48c810r_fan_data *data = f48c810r_fan_update_device(dev);
    int error, value;
                              ...
                              const char *buf, size_t count)
{
    ...
    f48c810r_fan_write_value(client, 0x33, 0); /* Disable fan speed watch dog */
    f48c810r_fan_write_value(client, fan_reg[FAN_DUTY_CYCLE_PERCENTAGE], duty_cycle_to_reg_val(value));
    return count;
}
...
static const struct attribute_group f48c810r_fan_group = {
    .attrs = f48c810r_fan_attributes,
};

static struct f48c810r_fan_data *f48c810r_fan_update_device(struct device *dev)
{
    struct i2c_client *client = to_i2c_client(dev);
    struct f48c810r_fan_data *data = i2c_get_clientdata(client);
    ...
    if (time_after(jiffies, data->last_updated + HZ + HZ / 2) ||
        ...
        dev_dbg(&client->dev, "Starting f48c810r_fan update\n");
        data->valid = 0;

        /* Update fan data
         */
        for (i = 0; i < ARRAY_SIZE(data->reg_val); i++) {
            int status = f48c810r_fan_read_value(client, fan_reg[i]);
...
static int f48c810r_fan_probe(struct i2c_client *client)
{
    struct f48c810r_fan_data *data;
    int status;
    ...
    data = kzalloc(sizeof(struct f48c810r_fan_data), GFP_KERNEL);
    if (!data) {
        status = -ENOMEM;
        goto exit;
    }
    ...
    status = sysfs_create_group(&client->dev.kobj, &f48c810r_fan_group);
    if (status) {
        goto exit_free;
    }
...
exit_remove:
    sysfs_remove_group(&client->dev.kobj, &f48c810r_fan_group);
exit_free:
    kfree(data);
exit:
...
static void f48c810r_fan_remove(struct i2c_client *client)
{
    struct f48c810r_fan_data *data = i2c_get_clientdata(client);
...
static const struct i2c_device_id f48c810r_fan_id[] = {
    { "f48c810r_fan", 0 },
    {}
};
MODULE_DEVICE_TABLE(i2c, f48c810r_fan_id);

static struct i2c_driver f48c810r_fan_driver = {
    .class        = I2C_CLASS_HWMON,
    .driver = {
        .name     = DRVNAME,
    },
    .probe        = f48c810r_fan_probe,
    .remove       = f48c810r_fan_remove,
    .id_table     = f48c810r_fan_id,
    .address_list = normal_i2c,
};

static int __init f48c810r_fan_init(void)
{
    return i2c_add_driver(&f48c810r_fan_driver);
}

static void __exit f48c810r_fan_exit(void)
{
    i2c_del_driver(&f48c810r_fan_driver);
}

module_init(f48c810r_fan_init);
module_exit(f48c810r_fan_exit);

MODULE_AUTHOR("Brandon Chuang <brandon_chuang@netweb.com.tw>");
MODULE_DESCRIPTION("f48c810r_fan driver");
MODULE_LICENSE("GPL");
