...
CFGGEN_PARAMS=" \
    ...
    -t ${SNAP}/usr/share/sonic/templates/frr_vars.j2 \
    -t ${SNAP}/usr/share/sonic/templates/supervisord/supervisord.conf.j2,/etc/supervisor/conf.d/supervisord.conf \
    -t ${SNAP}/usr/share/sonic/templates/supervisord/critical_processes.j2,/etc/supervisor/critical_processes \
    -t ${SNAP}/usr/share/sonic/templates/isolate.j2,/usr/sbin/bgp-isolate \
    -t ${SNAP}/usr/share/sonic/templates/unisolate.j2,/usr/sbin/bgp-unisolate \
"
...
if [ -z "$CONFIG_TYPE" ] || [ "$CONFIG_TYPE" == "separated" ]; then
    CFGGEN_PARAMS=" \
        -d \
        -y /etc/sonic/constants.yml \
        -t ${SNAP}/usr/share/sonic/templates/bgpd/gen_bgpd.conf.j2,/etc/frr/bgpd.conf \
        -t ${SNAP}/usr/share/sonic/templates/zebra/zebra.conf.j2,/etc/frr/zebra.conf \
        -t ${SNAP}/usr/share/sonic/templates/staticd/gen_staticd.conf.j2,/etc/frr/staticd.conf \
    "
    ...
    if [ -n "$MGMT_FRAMEWORK_CONFIG" ] && [ "$MGMT_FRAMEWORK_CONFIG" != "false" ]; then
        CFGGEN_PARAMS="$CFGGEN_PARAMS \
            -t ${SNAP}/usr/local/sonic/frrcfgd/bfdd.conf.j2,/etc/frr/bfdd.conf \
            -t ${SNAP}/usr/local/sonic/frrcfgd/ospfd.conf.j2,/etc/frr/ospfd.conf \
        "
    else
    ...
    CFGGEN_PARAMS=" \
        -d \
        -y /etc/sonic/constants.yml \
        -t ${SNAP}/usr/share/sonic/templates/gen_frr.conf.j2,/etc/frr/frr.conf \
    "
    sonic-cfggen $CFGGEN_PARAMS
    echo "service integrated-vtysh-config" > /etc/frr/vtysh.conf
    rm -f /etc/frr/bgpd.conf /etc/frr/zebra.conf /etc/frr/staticd.conf \
